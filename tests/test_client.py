...
import pytest
import agents.client as client  # import the module object
...
async def test_run_agents_returns_rows(monkeypatch):
    ...
    monkeypatch.setattr(client, "app", DummyApp())  # patch the module attribute
...
async def test_run_agents_respects_top_n(monkeypatch):
    ...
    monkeypatch.setattr(client, "app", DummyApp())
